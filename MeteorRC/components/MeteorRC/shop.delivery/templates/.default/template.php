...
<?foreach ($arResult as $id => $payment) {?>
	<div>
				<div class="radio">
					<label>
						<input type="<?if($arParams["MULTY"] == "Y"){?>radio<?}else{?>checkbox<?}?>" name="ORDER[DELIVERY_SYSTEM_ID]" id="optionsRadios<?=$payment["ID"]?>" value="<?=$payment["ID"]?>" <?if(isset($payment["DEFAULT"]) and $payment["DEFAULT"] == "Y"){?>checked=""<?}?>>
						<?=$payment["NAME"]?>
					</label>
				</div>
	</div>
