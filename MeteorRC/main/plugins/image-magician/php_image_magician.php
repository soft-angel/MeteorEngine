...
## --------------------------------------------------------
  ...
  # Date:     27-02-08    
    ...
    #
    {
    if (!$this->testGDInstalled()) { if ($this->debug) { die('The GD Library is not installed.'); }else{ return false; }};
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/ 
  ...
  #       forceStretch can be applied to options 1,2,3 and 4  
    ...
    if (!$this->image) { if ($this->debug) { die('file ' . $this->getFileName() .' is missing or invalid'); }else{ return false; }};
...
## -------------------------------------------------------- 
    ...
    if (!file_exists($file) && !$this->checkStringStartsWith('http://', $file)) { if ($this->debug) { die('Image not found.'); }else{ return false; }};
...
## -------------------------------------------------------- 
  ...
  #       third party class to save as bmp.
    ...
    if (!is_resource($this->imageResized)) { if ($this->debug) { die('saveImage: This is not a resource.'); }else{ return false; }}      
    $fileInfoArray = pathInfo($savePath);
    clearstatcache();
    if (!is_writable($fileInfoArray['dirname'])) {  if ($this->debug) { die('The path is not writable. Please check your permissions.'); }else{ return false; }} 
